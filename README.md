...
`content/publications.md` is generated by `update-pubs.py` using `publication-dois.txt`; do not modify this file by hand.

Prepend either doi or doi url to publication-doi.txt. Line starting with '#' is treated as comment and ignored.

In update-pubs.py, we query crossref.org API for info and write them as list to content/publications.md. Run this by
...
```sh
Line 4: DOI ajdjfjlllls not found.
```
